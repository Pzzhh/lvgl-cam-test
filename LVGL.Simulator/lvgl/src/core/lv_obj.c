...
bool lv_obj_is_valid(const lv_obj_t* obj)
{
    ...
    while (disp) {
        ...
        for (i = 0; i < disp->screen_cnt; i++) {
            ...
            lv_obj_t* parent = disp->screens[i];
            if (parent->spec_attr && parent->spec_attr->child_cnt && parent->spec_attr->children[0] == obj)
            {
                found = 1;
            }
            else
            {
                ...
                found = obj_valid_child(disp->screens[i], obj);
            }
...
inline static bool obj_valid_child(const lv_obj_t* parent, const lv_obj_t* obj_to_find)
{
